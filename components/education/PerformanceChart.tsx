...
export function PerformanceChart() {
  ...
  return (
    <section className="relative py-24 overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-[0.08]" style={{ backgroundImage: "radial-gradient(circle, #29ABE2 1px, transparent 1px)", backgroundSize: "32px 32px" }} />
      ...
      <div className="max-site relative z-10">
        <AnimateOnScroll>
          <div className="text-center mb-12">
            ...
            <p className="text-[#5D5F66] max-w-2xl mx-auto">
        ...
        <AnimateOnScroll delay={0.1}>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
            {[
              ...
              <div key={stat.label} className="rounded-xl bg-white border border-slate-200 shadow-sm p-4 text-center">
                <stat.icon className={`w-5 h-5 mx-auto mb-2 text-${stat.color}-500`} />
                <p className="text-2xl font-bold text-[#1B3A6B]">{stat.value}</p>
                <span className="text-xs text-[#A0A2A5]">{stat.label}</span>
              </div>
            ))}
          </div>
        </AnimateOnScroll>
        ...
        <AnimateOnScroll delay={0.2}>
          <div className="rounded-2xl bg-white border border-slate-200 shadow-sm p-6">
            <h3 className="text-[#1B3A6B] font-bold mb-6">Monthly P&L (%)</h3>
              ...
              {performanceData.map((d, i) => {
                ...
                return (
                  ...
                  >
                    {/* Tooltip */}
                    {isHovered && (
                      <div className="absolute -top-20 left-1/2 -translate-x-1/2 bg-[#1B3A6B] border border-[#1B3A6B] rounded-lg px-3 py-2 text-xs whitespace-nowrap z-10 shadow-lg">
                        <p className="text-white font-bold">{d.month}: {d.pnl > 0 ? "+" : ""}{d.pnl}%</p>
                        <p className="text-white/70">{d.trades} trades | {d.winRate}% win rate</p>
                        <p className="text-white/70">Max DD: -{d.dd}%</p>
                      </div>
                    )}
                    ...
                    <span className="text-[10px] text-[#A0A2A5] mt-2 font-mono">{d.month}</span>
                  </div>
                );
              })}
            </div>
            ...
            {/* Insight */}
            <div className="mt-6 p-4 rounded-xl bg-[#29ABE2]/5 border border-[#29ABE2]/15">
              <p className="text-[#5D5F66] text-sm leading-relaxed">
                <strong className="text-[#1B3A6B]">Key Lesson:</strong> Even a profitable strategy with a +43.3% annual return has 3 losing months.
